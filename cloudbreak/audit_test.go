...
import (
	...
	"github.com/hortonworks/cb-cli/cloudbreak/v2/client_cloudbreak/v3_workspace_id_audits"
	...
	resp := []*models_cloudbreak.AuditEvent{
		{
			AuditID:  1,
			Duration: 2,
			Operation: &models_cloudbreak.OperationDetails{
				Account:           "seq1234567",
...
func (*mockAuditClient) GetAuditEventByWorkspace(params *v3_workspace_id_audits.GetAuditEventByWorkspaceParams) (*v3_workspace_id_audits.GetAuditEventByWorkspaceOK, error) {
	...
	resp := &models_cloudbreak.AuditEvent{
		...
		RawFlowEvent: &models_cloudbreak.StructuredFlowEvent{
			Blueprint: &models_cloudbreak.BlueprintDetails{
